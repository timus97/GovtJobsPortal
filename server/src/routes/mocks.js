...
meMocksRouter.post('/mocks/:seriesId/attempts', studentAuth.requireStudent, async (req, res) => {
  const result = await mockAttempts.startAttempt(req.student.uid, req.params.seriesId, req.body || {});
...
meMocksRouter.get('/mocks/attempts/:attemptId', studentAuth.requireStudent, async (req, res) => {
  const result = await mockAttempts.getAttempt(req.student.uid, req.params.attemptId);
  if (result.error === 'NOT_FOUND') return res.status(404).json({ error: 'Attempt not found' });
  res.json(result);
});

meMocksRouter.post('/mocks/attempts/:attemptId/submit', studentAuth.requireStudent, async (req, res) => {
  const answers = req.body && typeof req.body === 'object' ? req.body.answers : {};
  const result = await mockAttempts.submitAttempt(req.student.uid, req.params.attemptId, answers);
  if (result.error === 'NOT_FOUND') return res.status(404).json({ error: 'Attempt not found' });
